...
const insertMovies = (movies: Movie[]) => {
  return (
    ...
    movies.map(movie => `('${escape(movie.imdbId)}', ${movie.popularity}, ${movie.budget}, ${movie.budgetAdjusted}, ${movie.revenue},
    ${movie.revenueAdjusted}, '${escape(movie.originalTitle)}', '${escape(movie.homepage)}', '${escape(movie.tagline ? movie.tagline : ' ')}', '${escape(movie.overview)}', 
    ${movie.runtime}, '${escape(movie.releaseDate)}')`).join(",")
  );
};
